...
describe('calcOnahBeinonit', () => {
  ...
  /** Yireim keeps both day and night onot on that Hebrew date (two results). */
  it('yireim: day and night onot on the thirtieth Hebrew day', () => {
    const results = calcOnahBeinonit(anchor, 'yireim');
  ...
  it('chabad: night start yields day + night on Hebrew day 30; day start yields one onah', () => {
    ...
    const nightResults = calcOnahBeinonit(nightAnchor, 'chabad');
    expect(nightResults).toHaveLength(2);
    expect(nightResults.map((r) => r.onah).sort()).toEqual(['day', 'night']);

    const dayResults = calcOnahBeinonit(anchor, 'chabad');
